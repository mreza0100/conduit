...
#
# ╔═══════════════════════════════════════════════════════════════════╗
# ║      🚀 CONDUIT MANAGER (Iran-Only) v1.2                             ║
# ║                                                                   ║
...
VERSION="1.2"
CONDUIT_IMAGE="conduit:distroless"
...
print_header() {
    echo -e "${CYAN}"
    echo "╔═══════════════════════════════════════════════════════════════════╗"
    echo "║                🚀 CONDUIT MANAGER (Iran-Only) v${VERSION}                  ║"
    echo "╠═══════════════════════════════════════════════════════════════════╣"
...
run_conduit() {
    ...
    log_info "Using local image: $CONDUIT_IMAGE"
    if ! docker image inspect "$CONDUIT_IMAGE" &>/dev/null; then
        log_error "Local image $CONDUIT_IMAGE not found. Build it first."
        exit 1
    fi
    ...
    for i in $(seq 1 $count); do
        ...
        docker run -d \
            ...
            $resource_args \
            "$CONDUIT_IMAGE" \
            start --max-clients "$MAX_CLIENTS" --bandwidth "$BANDWIDTH" --allowed-countries IR --stats-file -v
...
run_conduit_container() {
    ...
    docker run -d \
        ...
        $resource_args \
        "$CONDUIT_IMAGE" \
        start --max-clients "$mc" --bandwidth "$bw" --allowed-countries IR --stats-file -v
}

print_header() {
    echo -e "${CYAN}"
    echo "╔═══════════════════════════════════════════════════════════════════╗"
    printf "║                🚀 CONDUIT MANAGER (Iran-Only) v%-5s                  ║\n" "${VERSION}"
    echo "╚═══════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
print_live_stats_header() {
    local EL="\033[K"
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════════╗${EL}"
    printf "║  ${NC}🚀 CONDUIT MANAGER (Iran-Only) v%-5s   ${CYAN}CONDUIT LIVE STATISTICS      ║${EL}\n" "${VERSION}"
    echo -e "╠═══════════════════════════════════════════════════════════════════╣${EL}"
...
show_about() {
    clear
    echo -e "${CYAN}══════════════════════════════════════════════════════════════════${NC}"
    echo -e "              ${BOLD}ABOUT CONDUIT MANAGER (Iran-Only)${NC}"
    echo -e "${CYAN}══════════════════════════════════════════════════════════════════${NC}"
    echo ""
...
update_conduit() {
    ...
    local pull_output
    pull_output="Using local image - no pull needed"
    local pull_status=$?
    echo "$pull_output"
